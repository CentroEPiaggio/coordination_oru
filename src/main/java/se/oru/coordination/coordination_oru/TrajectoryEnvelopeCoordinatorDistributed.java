 ...
 *
 */
public abstract class TrajectoryEnvelopeCoordinatorDistributed extends AbstractTrajectoryEnvelopeCoordinator {
	 ...
	 */
	public TrajectoryEnvelopeCoordinatorDistributed() {
		this(1000, 1000.0);
	}
	 ...
	 */
	public TrajectoryEnvelopeCoordinatorDistributed(int CONTROL_PERIOD, double TEMPORAL_RESOLUTION) {
		...
		for (Integer robotID : robotIDs) {						
			if (allDeps.containsKey(robotID) || artificialDeps.containsKey(robotID)) {
				...
				if (allDeps.containsKey(robotID)){
					...
					if(dep.getDrivingTrajectoryEnvelope() != null) {
				    
					RobotReport rr1 = currentReports.get(dep.getWaitingRobotID());
					RobotReport rr2 = currentReports.get(dep.getDrivingRobotID());
					Pose waitp = dep.getWaitingPose();
					
					//Compute Distrance between robots
					double distance_dep = 
							Math.sqrt( 
								Math.pow(rr1.getPose().getX() - rr2.getPose().getX(), 2) 
								+ Math.pow(rr1.getPose().getY() - rr2.getPose().getY(), 2));
					
					//Compuite distance between the waiting robot and the waiting point
					double dist_wait = 
							Math.sqrt( 
									Math.pow(waitp.getX() - rr1.getPose().getX(), 2) 
									+ Math.pow(waitp.getY() - rr1.getPose().getY(), 2));

						if(distance_dep < 10 || dist_wait < 5) firstDep = allDepsTree.get(robotID).first();
						//firstDep = allDepsTree.get(robotID).first();
				}
					else firstDep = allDepsTree.get(robotID).first();
				}
		...
		synchronized(solver) {
			...
			synchronized(allCriticalSections) {
				...
				for (CriticalSection cs : this.allCriticalSections) {
					...
					/* USEDO TO NOT COMPUTE THE DEPENDENCY THROWS AN ERROR IF BOTH ROBOT CANNOT STOP
					double distance = 
							Math.sqrt( 
					...
					boolean r1inCS = robotReport1.getPathIndex() > cs.getTe1Start() && robotReport1.getPathIndex() < cs.getTe1End();
					boolean r2inCS = robotReport2.getPathIndex() > cs.getTe2Start() && robotReport2.getPathIndex() < cs.getTe2End();
					if(!(r2inCS || r1inCS)) {

					if(Math.max(dist_from_cs_1,dist_from_cs_2) < 5){
					 
					}
					else continue;
					}
					else {
						if(Math.max(dist_from_cs_1,dist_from_cs_2) > 20){
							continue;
						}
					}
				}*/
			...
			synchronized(currentDependencies) {
				...
				for (int robotID : robotIDs) {
					...
					if (constrainedRobotIDs.containsKey(robotID)) {
						...
						//if(dep.getWaitingRobotID() != dep.getDrivingRobotID()) setCriticalPoint(dep.getWaitingRobotID(), -1, retransmitt);
			...
			synchronized(currentDependencies) {
				...
				for (int robotID : robotIDs) {
					...
					if (constrainedRobotIDs.containsKey(robotID)) {
						...
						setCriticalPoint(dep.getWaitingRobotID(), dep.getWaitingPoint(), retransmitt);
						//setCriticalPoint(dep.getWaitingRobotID(), -1, retransmitt);
