...
public abstract class TrajectoryEnvelopeTrackerRK4Distributed extends AbstractTrajectoryEnvelopeTracker implements Runnable {
	...
	public TrajectoryEnvelopeTrackerRK4Distributed(TrajectoryEnvelope te, int timeStep, double temporalResolution, TrajectoryEnvelopeCoordinatorSimulationDistributed tec, TrackingCallback cb) {
	...
	public TrajectoryEnvelopeTrackerRK4Distributed(TrajectoryEnvelope te, int timeStep, double temporalResolution, double maxVelocity, double maxAcceleration, TrajectoryEnvelopeCoordinatorDistributed tec, TrackingCallback cb) {
	...
	private void enqueueOneReport() {
		...
		synchronized (reportsList) {
			...
			if (timeNow - reportTimeLists.get(reportTimeLists.size()-1) > tec.getControlPeriod() + TrajectoryEnvelopeCoordinatorDistributed.MAX_TX_DELAY) { //the known delay
				...
				synchronized (externalCPCounter)
				{
					...
					while(trial < numberOfReplicas) {
						if (rand.nextDouble() < (1-NetworkConfiguration.PROBABILITY_OF_PACKET_LOSS)) //the real one
							...
							TrajectoryEnvelopeCoordinatorSimulationDistributed tc = (TrajectoryEnvelopeCoordinatorSimulationDistributed)tec;
