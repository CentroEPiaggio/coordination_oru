...
import se.oru.coordination.coordination_oru.simulation2D.TrajectoryEnvelopeCoordinatorSimulationDistributed;
import se.oru.coordination.coordination_oru.util.BrowserVisualization;
import se.oru.coordination.coordination_oru.util.JTSDrawingPanelVisualization;
import se.oru.coordination.coordination_oru.util.Missions;

@DemoDescription(desc = "Simple test showing the use of pre-planned paths stored in files. Test from UNIPI.")
public class TestTrajectoryEnvelopeCoordinatorThreeRobots_UNIPI_distributed {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		final TrajectoryEnvelopeCoordinatorSimulationDistributed tec = new TrajectoryEnvelopeCoordinatorSimulationDistributed(MAX_VEL,MAX_ACCEL);
		...
		tec.addComparator(new Comparator<RobotAtCriticalSection> () {
			@Override
			public int compare(RobotAtCriticalSection o1, RobotAtCriticalSection o2) {
				CriticalSection cs = o2.getCriticalSection();
                

				return (cs.getTe2().getPathLength()-cs.getTe1().getPathLength());
			}
		});
		...
		TrackingCallback cb_R1 = new TrackingCallback(null) {
			...
			public String[] onPositionUpdate() {
				...
				int critical = rr.getCriticalPoint();
				return new String[] {"R1:", rrStr, " Critical Point", Integer.toString(critical)};
			}
		...
		TrackingCallback cb_R2 = new TrackingCallback(null) {
			...
			public String[] onPositionUpdate() {
				// AbstractTrajectoryEnvelopeTracker tracker = trackers.get(robotID);
				// RobotReport rr = tracker.getRobotReport(); 
				RobotReport rr = tec.getRobotReport(2);
				String rrStr = String.valueOf(2);
		...
		TrackingCallback cb_R3 = new TrackingCallback(null) {
			...
			public String[] onPositionUpdate() {
				// AbstractTrajectoryEnvelopeTracker tracker = trackers.get(robotID);
				// RobotReport rr = tracker.getRobotReport(); 
				RobotReport rr = tec.getRobotReport(3);
				String rrStr = String.valueOf(3);
		...
		tec.addTrackingCallback(2, cb_R2);
		tec.addTrackingCallback(3, cb_R3);
